...
def mandel_cl(ctx, x, y, dx, dy, max_iter=2500, iter_steps=2, w=3840,
                h=2160, use_dtype='double'):
    """

    Parameters:


        use_dtype (str): specifies the data type (i.e. precision) to perform calculations with.
            Options include float, double, and longdouble
    ...
    """
    if use_dtype == 'longdouble':
        _cltype, _nptype = 'double', np.longdouble
    elif use_dtype == 'double':
        _cltype, _nptype = 'double', np.double
    else:
        _cltype, _nptype = 'float', np.float

    # _cltype, _nptype = ("double", np.float128) if use_double else ("float", np.float32)
    ...
    y0 = y - dy
    y1 = y + dy
    # x0 = x - ((Decimal(16) * zoom)/Decimal(2.))
    # y0 = y - ((Decimal(9) * zoom)/Decimal(2.))
    # x1 = x + ((Decimal(16) * zoom)/Decimal(2.))
    # y1 = y + ((Decimal(9) * zoom)/Decimal(2.))

    # Create index map in x,y pairs
    xx = np.arange(0, w, 1, dtype=np.uint32)
    yy = np.arange(0, h, 1, dtype=np.uint32)
    ...
    coord_map[:] = index_map
    coord_map[:] *= (_nptype((x1-x0)/Decimal(w)), _nptype((y1-y0)/Decimal(h)))
    ...
    buffer_zin_cl = cl.Buffer(ctx, mf.READ_ONLY, size=coord_map.nbytes)
    buffer_out = np.zeros(w*h, dtype=np.uint32) # This will contain the iteration values of that run
    buffer_zout = np.zeros(w*h, dtype=np.float64) # same thing but for z values
    buffer_zout_cl = cl.Buffer(ctx, mf.WRITE_ONLY, size=buffer_zout.nbytes) # for z values
    buffer_out_cl = cl.Buffer(ctx, mf.WRITE_ONLY, size=buffer_out.nbytes)
    buffer_out_coords = np.zeros(w*h*2, dtype=_nptype) # This the last x,y values
    ...
    #iter_map = np.zeros(w*h, dtype=np.uint32).reshape((w, h)) #.reshape((h, w))
    iter_map = np.zeros(w*h, dtype=np.uint32).reshape((h, w))
    zval_map = np.zeros(w*h, dtype=np.float64).reshape((h, w))
    ...
    while to_do > 0 and start_max_iter < max_iter:
        ...
        cl.enqueue_copy(cl_queue, buffer_zin_cl, coord_map[:to_do*2]).wait()
        ...
        cl.enqueue_copy(cl_queue, buffer_zout, buffer_zout_cl).wait()
        ...
        zval_map[tmp[:,1], tmp[:,0]] = buffer_zout[done]
    ...
    # convert iterations/zvals to be continuous
    scaled_zvals = zval_map.reshape((h, w))
    scaled_zvals = np.log(np.log(np.sqrt(scaled_zvals)+1)+1)/5
    # scaled_zvals = np.log(scaled_zvals+1)
    # positive = scaled_zvals > 0
    # scaled_zvals[positive] = np.log(scaled_zvals[positive])/np.log(2)
    # scaled_zvals[scaled_zvals==np.inf] = np.max(scaled_zvals[scaled_zvals<np.inf])
    # scaled_zvals[scaled_zvals < 0] = 0

    # grab the iter_map
    esc_iters = iter_map.reshape((h, w))

    # if np.max(esc_iters) < np.max(scaled_zvals):
    #     scaled_zvals = scaled_zvals*(np.max(esc_iters)/np.max(scaled_zvals))/2

    esc_rates = esc_iters - scaled_zvals
